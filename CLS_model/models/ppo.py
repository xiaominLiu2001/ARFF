...
class ActorCritic(nn.Module):
    ...
    def act(self, state_ini, memory, restart_batch=False, training=False):
        ...
        Encoder[batch_size, 512] Dimensionality reduction of feature dimensions
        GRU[1, batch_size, 512] Maintenance of temporal information
        Actor[batch_size, K] Generation of Action Space
        Sample & Clip[batch_size, K] Balance between exploration and utilization (through normal distribution sampling)
        '''
        if restart_batch:
            del memory.hidden[:]
...
class PPO:
    ...
    def update(self, memory):
        ...
        rewards = [reward.unsqueeze(0) if reward.dim() == 0 else reward for reward in rewards]
...
class CL(nn.Module):
    ...
    def forward(self, x_views):
        assert isinstance(x_views, list), f"x_views is not list"
