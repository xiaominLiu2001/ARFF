...
class BestVariable:
    ...
    def compare(self, val, epoch=None, inplace=False):
        flag = True if (self.order == 'max' and val >= self.best) or (
                    self.order == 'min' and val <= self.best) else False
        if flag and inplace:
            self.best = val
    ...
    def isequal(self,val, inplace=False):
        if val == self.best:
            return 0
        elif val > self.best:
            if inplace:
...
def save_equal_checkpoint(state, checkpoint, epoch, test_acc):
       torch.save(state, os.path.join(checkpoint, f'model_best_epoch{epoch}_{test_acc}.pth.tar'))


def save_valid_checkpoint(state, checkpoint, epoch, valid_acc):
    torch.save(state, os.path.join(checkpoint, f'model_best_epoch{epoch}_{valid_acc}.pth.tar'))
